...
export class ProfessionalGantt {
  ...
  bindControls() {
    this.zoomControls?.addEventListener('click', (event) => {
      ...
      if (this.lastPayload) this.render(this.lastPayload, { preserveCenter: true });
    });
    ...
    let html = `
      ...
      >
        <div class="gantt-corner gantt-sticky-top gantt-sticky-left">
          ...
          <small>${this.escapeHtml(ZOOM[this.zoom].label)} · rango global</small>
        </div>
          ...
          stage.items.forEach((activity) => {
            ...
            html += `
              <div class="gantt-task-label gantt-sticky-left">
                ...
                <small>${this.escapeHtml(dateText)} · ${activity.unidades_cumplen}/${activity.total_unidades} unidades</small>
              </div>
            `;
            ...
            html += `
              <div class="gantt-timeline-row" style="${background}">
                ...
                <button
                  ...
                  data-gantt-tooltip="${this.escapeHtml(encodeURIComponent(tooltip))}"
                  title="${this.escapeHtml(`${activity.actividad} · ${activity.avance}% · ${statusLabels[evaluation.status]}`)}"
                  aria-label="${this.escapeHtml(
                    `${activity.actividad}. ${dateText}. ${activity.avance}% de avance. ${statusLabels[evaluation.status]}`,
                  )}"
                >
    ...
    requestAnimationFrame(() => {
      ...
      if (options.preserveCenter) {
        this.root.scrollLeft = Math.max(
          0,
          Math.min(max, previousCenter * this.root.scrollWidth - this.root.clientWidth / 2),
        );
  ...
  getDomain(allRows) {
    ...
    const starts = scheduled.map((row) => this.parseDate(row.inicio)).filter(Boolean);
    const ends = scheduled.map((row) => this.parseDate(row.fin_plan)).filter(Boolean);
    ...
    [...rows]
      ...
      .forEach((activity) => {
        if (!cells.has(activity.celula)) {
          cells.set(activity.celula, { celula: activity.celula, entities: new Map() });
        }
        const cell = cells.get(activity.celula);
        if (!cell.entities.has(activity.entidad)) {
          cell.entities.set(activity.entidad, { entidad: activity.entidad, stages: new Map() });
        }
        const entity = cell.entities.get(activity.entidad);
        if (!entity.stages.has(activity.etapa)) {
          entity.stages.set(activity.etapa, { etapa: activity.etapa, items: [] });
        }
  ...
  groupRow(label, type) {
    ...
    return `
      <div class="gantt-group-label gantt-group-${type} gantt-sticky-left">${prefix}${this.escapeHtml(label)}</div>
  ...
  buildCalendar(minDate, maxDate, totalDays) {
    ...
    while (yearCursor <= maxDate) {
      ...
      years.push(this.calendarCell('year', start, end, minDate, totalDays, String(start.getFullYear())));
      yearCursor = next;
    }
    ...
    while (monthCursor <= maxDate) {
      ...
      months.push(this.calendarCell('month', start, end, minDate, totalDays, label));
      monthCursor = next;
    }
    ...
    if (this.zoom === 'day') {
      ...
      while (cursor <= maxDate) {
        ...
        units.push(this.calendarCell('day', cursor, next, minDate, totalDays, label));
        cursor = next;
        index++;
      }
    } else {
      ...
      const interval = this.zoom === 'month' ? 4 : totalDays > 365 ? 3 : totalDays > 180 ? 2 : 1;
      let index = 0;
      while (cursor <= maxDate) {
        const next = new Date(cursor);
        ...
        const label =
          index % interval === 0
            ...
            : '';
        units.push(this.calendarCell('week', start, end, minDate, totalDays, label, true));
        cursor = next;
        index++;
      }
    }
  ...
  calendarCell(type, start, end, minDate, totalDays, label, vertical = false) {
    ...
    return `
      <div class="gantt-calendar-cell gantt-calendar-${type}" style="left:${left}%;width:${width}%">
        ${label ? `<span class="${vertical ? 'is-vertical' : ''}">${this.escapeHtml(label)}</span>` : ''}
      </div>
    `;
  }
  ...
  buildTimelineBackground(minDate, maxDate, totalDays) {
    ...
    while (cursor <= maxDate) {
      if (cursor.getDay() === 6) {
        ...
        gradients.push(
          `linear-gradient(to right, transparent 0 ${Math.max(0, left)}%, rgba(92,108,102,.055) ${Math.max(0, left)}% ${Math.min(100, right)}%, transparent ${Math.min(100, right)}% 100%)`,
        );
      }
    ...
    gradients.push(
      `repeating-linear-gradient(to right, transparent 0 ${Math.max(1, weekPx - 1)}px, rgba(113,136,127,.12) ${Math.max(1, weekPx - 1)}px ${weekPx}px)`,
    );
    ...
    rows.forEach((row) => {
      html += `
        <div class="gantt-pending-only-item">
          ...
          <span>${this.escapeHtml(row.entidad)} · ${this.escapeHtml(row.celula)}</span>
  ...
  scrollToReference() {
    ...
    const position = this.diffDays(domain.minDate, reference) / domain.totalDays;
  ...
  showTooltip(bar, x, y) {
    ...
    let data;
    try {
      data = JSON.parse(decodeURIComponent(bar.dataset.ganttTooltip || '%7B%7D'));
    } catch {
      return;
    }
    ...
    tooltip.innerHTML = `
      ...
      <span>${this.escapeHtml(data.entidad || '')} · ${this.escapeHtml(data.celula || '')}</span>
      <dl>
        <div><dt>Inicio</dt><dd>${this.escapeHtml(data.inicio || '—')}</dd></div>
        <div><dt>Fin</dt><dd>${this.escapeHtml(data.fin || '—')}</dd></div>
        <div><dt>Avance</dt><dd>${this.escapeHtml(data.avance || '—')}</dd></div>
        <div><dt>Estado</dt><dd>${this.escapeHtml(data.estado || '—')}</dd></div>
      </dl>
  ...
  positionTooltip(x, y) {
    ...
    let left = x + 16;
    let top = y + 16;
    if (left + rect.width > window.innerWidth - padding) left = x - rect.width - 16;
    if (top + rect.height > window.innerHeight - padding) top = y - rect.height - 16;
